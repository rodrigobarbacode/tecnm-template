...
{ 
    ...
    $consulta = "SELECT actividades.noControl, alumno.paterno, alumno.materno, alumno.nombre, extraescolar.Actividad, extraescolar.Grupo, extraescolar.Horario, extraescolar.Docente, actividades.Estatus
	from actividades 
	...
	while($ver = mysqli_fetch_row($result)){
		$objPHPExcel->getActiveSheet()->setCellValue('A6',$ver[4]);
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $ver[0]);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $ver[1]." ".$ver[2]." ".$ver[3]);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $ver[5]);
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $ver[6]);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, $ver[7]);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$fila, $ver[8]);

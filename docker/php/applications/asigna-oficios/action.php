...
if($received_data->action == 'fetchall')
{
    if($rol == 1){
        $query = "
        SELECT * FROM folios where fecha >= '2025-01-01'
...
if($received_data->action == 'fetchSingleUpdate')
{
 $query = "
 SELECT * FROM folios
 WHERE id_folio = '".$received_data->id."'
 ";
 ...
 foreach($result as $row)
 {
  $data['id_folio'] = $row['id_folio'];
  $data['tipo'] = $row['tipo'];
  $data['dirigido'] = $row['dirigido'];
  $data['asunto'] = $row['asunto'];
  $data['fk_departamento'] = $row['fk_departamento'];
 }
...
if($received_data->action == 'update')
{
    $data = array(
        ':dirigido' => $received_data->Dirigido,
        ':asunto' => $received_data->Asunto,
        ':id_folio' => $received_data->Folio
       );
      
    $query = "
       UPDATE folios 
       SET dirigido = :dirigido, asunto = :asunto 

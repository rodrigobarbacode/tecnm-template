<?php
	session_start();
	//$conn = new mysqli("mariadb-php", "root", "Tzuput4r0", "tec40");
	$connect = new PDO("mysql:host=mariadb-php;dbname=aoc", "oficios", "sysOficios.23");
     ...
     <div class="table-responsive">
      <table class="table table-bordered table-striped">
       ...
       <tr v-for="row in allData">
        ...
        <?php
          if($rol == 1){
            echo '<td><button type="button" name="edit" class="btn btn-primary btn-xs edit" @click="updateFolio(row.id_folio)">Editar</button></td>
            <td><button type="button" name="delete" class="btn btn-danger btn-xs delete" @click="deleteData(row.id_folio, row.tipo, row.folio)">Eliminar</button></td>';
          }
        ?>
        ...
        <div class="modal-content">
         ...
         <div class="modal-body">
          ...
          if($rol == 1){
            echo '
          <div class="form-group">
           <label v-if="hiddenId == 0">Departamento:</label>
           <select class="form-control input-lg" v-model="select_depto" v-if="hiddenId == 0">
          ...
          <br />
          <div class="form-group">
            <input type="hidden" class="form-control" v-model="hiddenId"/>
          </div>
          <div align="center">
...
var application = new Vue({
 ...
 methods:{
  ...
  submitDataAdmin:function(){
   ...
   else if(application.actionButton == 'Actualizar')
    {
     axios.post('action.php', {
      action:'update',
      Asunto : application.asunto,
      Dirigido : application.dirigido,
      Folio : application.hiddenId
     }).then(function(response){
      application.myModel = false;
      application.fetchAllData();
      application.asunto = '';
  ...
  updateFolio:function(id_folio){
   axios.post('action.php', {
    action:'fetchSingleUpdate',
    id:id_folio
   }).then(function(response){
    application.dirigido = response.data.dirigido;
    application.asunto = response.data.asunto;
    application.actionButton = "Actualizar";
    application.dynamicTitle = "Editar";
    application.hiddenId = response.data.id_folio;
    application.myModel = true;
   });
  },
  deleteData:function(id_folio, tipo, folio){
   if(confirm("Se Eliminara el Registro " +tipo+ " No. " +folio))
   {
    axios.post('action.php', {
     action:'delete',
     id:id_folio

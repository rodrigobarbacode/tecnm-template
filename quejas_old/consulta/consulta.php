...
if (isset($_SESSION["usuario_valido"]))
   ...
   // Calcular el número total de filas de la tabla
        $instruccion = "select FECHA_QUEJA,CONTROL, ATENDIDA, FECHA_RESP from quejasAlumnos where ATENDIDA = 'NO';";
        $consulta = mysql_query ($instruccion, $conexion);
        if (!$consulta) 
		{
		   print ("Falló la consulta, Error: " . mysql_error());
		   //print ("<P>[ <A HREF='logout.php'>Salir</A> ]</P>\n");
		   die ("");
		}
   ...
   // Enviar consulta
        ...
        $consulta = mysql_query ($instruccion, $conexion);
        $instruccion = "select * from quejasAlumnos where ATENDIDA = 'NO' limit $comienzo, $num";
        $consulta = mysql_query ($instruccion, $conexion);
        if (!$consulta) 
		{
		   print ("Falló la consulta, Error: " . mysql_error());
      ...
      if ($nfilas > 0)
      {
         ...
         for ($i=0; $i<$nfilas; $i++)
         {
            ...
            print ("<TR>\n");
            print ("<TD><font size=5><center><A HREF=\"actualizar1.php?fecha_queja=" . $resultado['FECHA_QUEJA'] . "\" TARGET=\"_blank\">".$resultado['FECHA_QUEJA']."</A> </TD>\n");
